...
class PanelProvider {
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('githubSwitcherPanel', 'GitHub Profile Switcher', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true,
        });
        this.panel.iconPath = vscode.Uri.joinPath(this.context.extensionUri, 'images', 'icon.png');
    ...
    async handleMessage(msg) {
        switch (msg.type) {
            ...
            case 'deleteProfile': {
                ...
                if (!profile)
                    break;
                const confirm = await vscode.window.showWarningMessage(`Are you sure you want to delete the profile "${profile.label}"? This cannot be undone.`, { modal: true }, 'Delete');
                if (confirm === 'Delete') {
