  ...
  private async handleMessage(msg: WebviewMessage): Promise<void> {
    switch (msg.type) {
      ...
      case 'saveProfile': {
        ...
        try {
          let savedProfile: GitHubProfile;
          if (data.id) {
      ...
      case 'copyProfilePublicKey': {
        ...
        if (profile && profile.sshKeyPath) {
          try {
            const { SshConfigManager } = require('../sshConfigManager') as typeof import('../sshConfigManager');
      ...
      case 'generateSshKey': {
        const { id, label, email } = msg.payload as { id?: string; label: string; email: string };
        try {
